...
struct PairingView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 28) {
                ...
                if let status = model.accountStatus, status != .available {
                    iCloudWarning(status)
                }
    ...
    private func iCloudWarning(_ status: CKAccountStatus) -> some View {
        Label {
            Text(SyncError.iCloudUnavailable(status).errorDescription ?? "")
                .font(Theme.rounded(14))
        } icon: {
    ...
    private var privacyNote: some View {
        Label {
            Text("Your statuses are end-to-end encrypted in your own iCloud. No servers, no accounts, no ads.")
